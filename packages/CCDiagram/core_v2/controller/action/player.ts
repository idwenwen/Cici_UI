import { defNoEnum, UUID } from "@cc/tools";
...
// 当前帧动画绘制计算内容，返回是否有下一帧内容
export type framePicture = (current: Duration) => boolean;
...
class Player {
  ...
  // 帧动画播放器加载
  loading(operation: framePicture, restart?: Function): player {
    // 播放初始化
    this.ready();
